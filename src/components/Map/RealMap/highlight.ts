...
L.Layer.Highlight = L.Layer.extend({
  ...
  onAdd: function (_map) {
    const layer = L.geoJSON(this._data, this._otherOpt);
  ...
  do: function (_searchOpt, _otherOpt = {}) {
    ...
    this._data = this._getHighlightData({
      street: _searchOpt.street,
      city: _searchOpt.city,
      q: _searchOpt.q,
    });
  ...
  _getHighlightData: function (_opt) {
    const result = this._getBatch(_opt);
    let i = 0;

    if (this._searchOpt.limit > 1) {
      while (
        this._filter(result).features.length % this._searchOpt.limit ===
        0
      ) {
  ...
  _handleOpts: function (_searchOpt, _otherOpt) {
    if (
      !_searchOpt ||
      (!_searchOpt.q && !_searchOpt.city && !_searchOpt.street)
    ) {
  ...
  _cleanup: function (_data) {
    if (this._searchOpt.filter) {
      _data.features = _data.features.filter(
        (e) => e.geometry.type == this._searchOpt.filter
      );
    }
  ...
  _filter: function (_data) {
    if (this._searchOpt.city) {
      _data.features = _data.features.filter(
        (e) =>
          e.properties.address.city == this._searchOpt.city ||
    ...
    if (this._searchOpt.street) {
      _data.features = _data.features.filter(
        (e) => e.properties.address.road == this._searchOpt.street
      );
    }
